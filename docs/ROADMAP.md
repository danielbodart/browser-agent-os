...
- *EOF model:* implicit close-on-writer-`proc_exit`. `LocalKernel.spawn` iterates the merged FdMap on worker exit and calls `transport.closeWriteEnd(end)` for every write end; consumers then observe a 0-byte read.
  - **2026-07-03 — refined to refcounting.** The naive "close every write end on exit" was correct only while each write end had a single holder. Once the shell hands its own stdout to a child (a *borrowed* write end, Stage 4), the child's exit would close the shell's stdout, muting every subsequent command in the session — a shared-stdout bug that no test caught because each shell test ran exactly one spawning command before `exit`. Fixed by refcounting write ends in the transport: `acquireWriteEnd` on spawn (once per write end in the process's fd table), `closeWriteEnd` decrements and only EOFs the buffer at zero (POSIX dup semantics). Ends never acquired — host-provided stdin, the kernel's default no-writer stdin — still close immediately. Regression coverage in `shell.test.ts` (sequential spawns; pipeline-then-command) and `shell-streaming.test.ts`.
...
- *Stdin reader bug:* `std.Io.File.stdin().reader(io, &rbuf).interface.readSliceShort` hangs after the first non-trivial read inside a JSPI worker. Worked around with a direct WASI `fd_read` extern. Worth filing upstream; for now the workaround is local to the shell and `ls` is unaffected (it only reads the directory fd, not stdin).
  - **2026-07-03 — re-verified; workaround must stay, and the hang is Chrome-JSPI-specific.** Swapping the shell back to the `readSliceShort` path passes *every Bun test*, including a staggered-stdin probe that forces a genuine block-then-resume across the JSPI boundary (`shell-streaming.test.ts`) — Bun's JSPI does not reproduce the hang. But all three `host-browser.smoke.test.ts` cases (real headless Chrome JSPI) time out at 15 s with the std reader and pass with the `fd_read` extern. So the bug is in Chrome's JSPI suspend/resume of the std reader's buffered read, not in the kernel or in Zig per se; Bun masks it. Do **not** remove the `readRaw` workaround on the strength of green Bun tests — the browser smoke suite is the only thing that exercises the real failure mode. (An earlier pass mistook this for premature-EOF, but that symptom was the shared-stdout close bug fixed below, not the reader.)
